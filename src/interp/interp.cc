...
Result Thread::Run(int num_instructions) {
  ...
  for (int i = 0; i < num_instructions; ++i) {
    ...
    switch (opcode) {
      ...
      case Opcode::TwoLocalGetI64Xor: {
        //printf("doing TwoLocalGetI64Xor!\n");
        /*
        case Opcode::I64Xor: {
          uint32_t top_minus_one = value_stack_top_ - 1;
          uint32_t top_minus_two = top_minus_one - 1;
        ...
        */
        ...
        //printf("got depth_1: %d!\n", depth_1);
        uint32_t depth_2 = ReadU32(&pc);
        //printf("got depth_2: %d!\n", depth_2);
        //printf("value_stack_top_: %d!\n", value_stack_top_);

        uint64_t rhs = value_stack_[value_stack_top_ - depth_1].i64;
        uint64_t lhs = value_stack_[value_stack_top_ - depth_2].i64;
        //printf("got rhs and lhs.\n");

        value_stack_[value_stack_top_] = MakeValue<uint64_t>(lhs ^ rhs);
        //printf("pushed result..\n");
